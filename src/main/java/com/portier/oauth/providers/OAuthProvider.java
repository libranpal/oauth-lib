...
package com.portier.oauth.providers;

import static com.portier.oauth.utils.OAuthUtils.generateRandomState;
...
import com.portier.oauth.SessionCache;
import com.portier.oauth.api.AuthenticationContext;
import com.portier.oauth.api.WebRequestClient;

/**
 * This class defines an OAuth client that provides authentication
 * to an authentication request.
 * @author sin
 */
public abstract class OAuthProvider
    implements WebRequestClient<Response>
{
  /**
   * The logger instance.
   */
  ...
  */
   ...
   */
  @Override
  public Response startAuthGrantFlow(
      final AuthenticationContext context)
  {
    log.debug("Starting the oauth grant flow");
    // Get a secured state.
    final String state = generateRandomState();
    log.debug("Generated random state = " + state);
    // WixCache.putService(this, new Date(), state, invocationProps.verb,
    // invocationProps.o);
    final OAuth2CodeGrantFlow codeGrantFlow =
        createCodeGrantFlow(state);
    SessionCache.addToCache(state, context);
    context.setOAuthFlow(codeGrantFlow);
    final String authURI = codeGrantFlow.start();
    log.debug("Authentication URI is " + authURI);
    final URI uri = UriBuilder.fromUri(authURI).build();
  ...
  /**
   * Creates oauth2 code grant flow.
   *
   * @param state The random state.
   * @return An OAuth2 code grant flow.
   */
  protected abstract OAuth2CodeGrantFlow createCodeGrantFlow(
      final String state);
}
