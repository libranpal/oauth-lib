 ...
 * beneficial ownership of such entity. "You" (or "Your") shall mean an
 ...
 * the Work and Derivative Works thereof. "Contribution" shall mean any work of
 * authorship, including the original version of the Work and any modifications
 * or additions to that Work or Derivative Works thereof, that is intentionally
 * submitted to Licensor for inclusion in the Work by the copyright owner or by
 * an individual or Legal Entity authorized to submit on behalf of the copyright
 ...
 * "Not a Contribution." "Contributor" shall mean Licensor and any individual or
 * Legal Entity on behalf of whom a Contribution has been received by Licensor
 * and subsequently incorporated within the Work. 2. Grant of Copyright License.
 ...
 * litigation is filed. 4. Redistribution. You may reproduce and distribute
 ...
 * any part of the Derivative Works; and (d) If the Work includes a "NOTICE"
 ...
 * part of the Derivative Works, in at least one of the following places: within
 * a NOTICE text file distributed as part of the Derivative Works; within the
 ...
 * Contributions. 6. Trademarks. This License does not grant permission to use
 ...
 * of the Work and reproducing the content of the NOTICE file. 7. Disclaimer of
 ...
 * exercise of permissions under this License. 8. Limitation of Liability. In no
 ...
 * such Contributor has been advised of the possibility of such damages. 9.
 ...
 * CONDITIONS APPENDIX: How to apply the Apache License to your work. To apply
 * the Apache License to your work, attach the following boilerplate notice,
 ...
 * third-party archives. Copyright [2017] [Kunal Sinha, Deo Jaiswal] Licensed
...
package com.portier.oauth.providers.outlook;

import static com.portier.oauth.providers.outlook.OutlookConstants.CLIENT_ID_ONEDRIVE;
import static com.portier.oauth.providers.outlook.OutlookConstants.CLIENT_SECRET_ONEDRIVE;
import static com.portier.oauth.providers.outlook.OutlookConstants.OUTLOOK_DRIVE_AUTHORIZE_URL;
import static com.portier.oauth.providers.outlook.OutlookConstants.OUTLOOK_DRIVE_TOKEN_URL;
import static com.portier.oauth.providers.outlook.OutlookConstants.SCOPE_ONEDRIVE;
import static com.portier.oauth.utils.OAuthUtils.REDIRECT_URI;
import static com.portier.oauth.utils.OAuthUtils.getConfigProperty;

import org.glassfish.jersey.client.oauth2.ClientIdentifier;
import org.glassfish.jersey.client.oauth2.OAuth2ClientSupport;
import org.glassfish.jersey.client.oauth2.OAuth2CodeGrantFlow;
import org.glassfish.jersey.client.oauth2.OAuth2FlowGoogleBuilder;
import org.glassfish.jersey.client.oauth2.OAuth2FlowGoogleBuilder.AccessType;
import org.glassfish.jersey.client.oauth2.OAuth2FlowGoogleBuilder.Prompt;
import org.glassfish.jersey.client.oauth2.OAuth2Parameters;

import com.portier.oauth.providers.OAuthProvider;

/**
 * This class defines a generic Outlook OAuth client.
 * @author DJ
 */
public final class OutlookDrive extends OAuthProvider
{
  /**
  ...
  public OutlookDrive()
  {
    // Initialize the client id and secret from the configuration properties.
    this.clientId = getConfigProperty(CLIENT_ID_ONEDRIVE);
    this.clientSecret = getConfigProperty(CLIENT_SECRET_ONEDRIVE);
    this.redirectURI = getConfigProperty(REDIRECT_URI);
  }
   ...
   */
  @Override
  protected OAuth2CodeGrantFlow createCodeGrantFlow(
      final String state)
  {
    log.debug("Creating the oauth code grant flow using the state ");
    log.debug("Outlook Drive Client ID ->" + getClientId());
    log.debug("Outlook Drive Client SECRET ->" + getClientSecret());
    log.debug("Outlook Drive authorization URL ->"
        + OUTLOOK_DRIVE_AUTHORIZE_URL);
    log.debug("Outlook Drive token URL ->" + OUTLOOK_DRIVE_TOKEN_URL);
    log.debug("Redirect URI ->" + getRedirectURI());
    return OAuth2ClientSupport
        .authorizationCodeGrantFlowBuilder(new ClientIdentifier(
            getClientId(),
            getClientSecret()),
            OUTLOOK_DRIVE_AUTHORIZE_URL,
            OUTLOOK_DRIVE_TOKEN_URL)
        .redirectUri(getRedirectURI())
        .property(OAuth2CodeGrantFlow.Phase.AUTHORIZATION,
            OAuth2Parameters.STATE, state)
        .property(OAuth2CodeGrantFlow.Phase.AUTHORIZATION,
            Prompt.getKey(),
            OAuth2FlowGoogleBuilder.Prompt.CONSENT.getValue())
        .property(OAuth2CodeGrantFlow.Phase.AUTHORIZATION,
            AccessType.getKey(), AccessType.OFFLINE.getValue())
        .property(OAuth2CodeGrantFlow.Phase.AUTHORIZATION,
            OAuth2Parameters.SCOPE, (SCOPE_ONEDRIVE))
        .property(OAuth2CodeGrantFlow.Phase.AUTHORIZATION, "resource",
