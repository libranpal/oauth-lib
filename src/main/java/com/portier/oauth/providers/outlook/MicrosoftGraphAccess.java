 ...
 * @author DJ
 */
public final class MicrosoftGraphAccess extends OAuthProvider
{
  /**
   * Creates an instance of this class.
   */
  public MicrosoftGraphAccess()
  {
      ...
      final String state)
  {
    ...
    log.debug("Onedrive Client ID ->" + getClientId());
    log.debug("Onedrive Client SECRET ->" + getClientSecret());
    log.debug("Outlook Drive authorization URL ->"
        + OutlookConstants.OUTLOOK_DRIVE_AUTHORIZE_URL);
    log.debug("Onedrive token URL ->"
        + OutlookConstants.OUTLOOK_DRIVE_TOKEN_URL);
    ...
    OAuth2CodeGrantFlow.Builder<?> builder = OAuth2ClientSupport
        ...
        .property(OAuth2CodeGrantFlow.Phase.AUTHORIZATION,
            OAuth2Parameters.SCOPE, (OutlookConstants.SCOPE_ONEDRIVE))
        .property(OAuth2CodeGrantFlow.Phase.AUTHORIZATION, "resource",
            "https://graph.microsoft.com/");
    this.properties.keySet().stream().forEach(
        x -> builder.property(OAuth2CodeGrantFlow.Phase.AUTHORIZATION,
            x, this.properties.get(x)));
