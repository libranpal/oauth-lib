...
package com.portier.oauth.callback;
...
/**
 * This class is invoked when the OAuth provider does a callback
 * to the server after successful authentication.
 * @author sin
 */
@Path("oauth2")
public final class AuthorizationCallback
{
  // The url info.
  @Context
  ...
  /**
   * The callback that is invoked when the authorization is successful at the oauth
   * service provider.
   *
      ...
      @QueryParam("state") final String state) throws Exception
  {
    ...
    final AuthenticationContext context =
        SessionCache.getAndRemoveFromCache(state);
    // Get the grant flow.
    final OAuth2CodeGrantFlow flow = context.getOAuthFlow();
    // The token result should be available after flow finishes.
    TokenResult tokenResult = null;
    // Retrieve the registered callback handler.
    final AuthenticationCompletionHandler handler =
        context.getCallbackHandler();
    try
    {
      tokenResult = flow.finish(code, state);
    }
    catch (Exception e)
    {
      return handler.handleError(e);
    }
    // The token result should be made available to the
    context.setTokenResult(tokenResult);
    // Let the service continue;
    return handler.handleCompletion(context);
  }
}
