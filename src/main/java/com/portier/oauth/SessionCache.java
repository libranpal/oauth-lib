...
/**
 * This method defines a cache that manages the relationship between the state
 * and the caller's context.
 *
 * FIXME: This class will be obsoleted when the session cache is available after
 * successful integration with the other components.
 * @author sin
 */
public final class SessionCache
{
  // The local cache.
  private static final ConcurrentHashMap<String, AuthenticationContext> CACHE =
   ...
   */
  public static void addToCache(final String state,
      final AuthenticationContext handler)
  {
    CACHE.put(state, handler);
  }



  /**
   * Retrieves the handler for a matching state. Should never return
   * null so let NPE happen so that we could figure it out.
   *
   * @param state The state value.
   * @return The oauthcallback handler.
   */
  public static AuthenticationContext getAndRemoveFromCache(
      final String state)
  {
